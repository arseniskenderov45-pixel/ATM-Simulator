...
class UserAccount {
    ...
    public UserAccount(String name, String pin) {
        this.name = name;
        this.pin = pin;
        this.balance = 0.0; // Стартовый баланс 0
        this.history = new ArrayList<>();
        addRecord("Аккаунт открыт");
    }

    public void addRecord(String record) {
        history.add(0, record); // Добавляем в начало (новые сверху)
...
class BankSystem {
    ...
    public String register(String name, String pin) {
        if (users.size() >= MAX_USERS) return "Ошибка: Банк переполнен (макс 10).";
        if (users.containsKey(name)) return "Ошибка: Имя занято.";
        if (pin.length() != 4 || !pin.matches("\\d+")) return "Ошибка: ПИН должен быть 4 цифры.";
...
public class Main extends Application {
    ...
    private StackPane screenContainer; // "Экран" банкомата

    // Поле, на котором сейчас фокус (для ввода с кнопок банкомата)
    private TextField activeInputField;

    @Override
    public void start(Stage primaryStage) {
        // Основной корпус банкомата
        ...
        // Эффект вдавленного экрана с рамкой
        ...
        // --- НИЗ (Клавиатура) ---
        VBox keypadArea = createKeypad();
        atmBody.setBottom(keypadArea);
    ...
    private void showRegisterScreen() {
        ...
        TextField pinField = new TextField(); // Обычный текст, чтобы видеть при создании
        ...
        btnCreate.setOnAction(e -> {
            ...
            if (result.equals("SUCCESS")) {
                showAlert("Успех", "Аккаунт создан! Теперь войдите.");
                showLoginScreen();
            } else {
    ...
    private void showHistoryScreen() {
        VBox layout = createScreenLayout();
        Label title = new Label("История операций (последние 10)");
    ...
    // --- ВСПОМОГАТЕЛЬНЫЕ ЭЛЕМЕНТЫ И ДИЗАЙН ---

    // Создание физической клавиатуры банкомата
    private VBox createKeypad() {
        ...
        btnClear.setOnAction(e -> {
            if (activeInputField != null) activeInputField.clear();
        });
        ...
        // Логика Enter сложнее, оставим просто как визуал или дубль клика по кнопке экрана
        ...
        if (!text.equals("C") && !text.equals("OK")) {
            btn.setOnAction(e -> {
                if (activeInputField != null) {
                    activeInputField.appendText(text);
                }
            });
        }
        return btn;
    ...
    private void setActiveFieldLogic(TextField field) {
        // Запоминаем, какое поле активно, чтобы клавиатура знала куда писать цифры
        field.setOnMouseClicked(e -> activeInputField = field);
        // По умолчанию ставим это поле активным при создании
